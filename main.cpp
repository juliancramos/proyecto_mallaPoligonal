...
int main() {
    ...
    while (comando != "salir") {
        ...
        // Comandos con un solo argumento
        if (partes.size() == 1) {
            if (comando == "ayuda") {
                ...
                try {
                    ...
                    if (figura != nullptr) {
                        ...
                        if (preFigEnvolvente.size() > 0) {
                            ...
                            std::cout << "Los vértices del objeto \"" << partes[1]
                            ...
                            for (int i = 0; i < preFigEnvolvente.size(); i++) {
                                std::cout << preFigEnvolvente[i] << "\t\t" << verticesCercanos[i] << "\t\t\t\t"
                                         << verticesCercanos[i].calcularDistancia(preFigEnvolvente[i].getX(),
                                                                               preFigEnvolvente[i].getY(),
                                                                               preFigEnvolvente[i].getZ()) << std::endl;
                            }
                        }
                    } else {
            ...
            if (comando == "guardar") {
                ...
                if (figura != nullptr) {
                    ...
                    try {
                        ...
                        if (centroide.getIndice() == -2) {
                            figura->setCentroide(centroide);
                            // figura->getGrafo().mostrarGrafo();
                            Vertice cercano = figura->verticeCercano(centroide.getX(), centroide.getY(), centroide.getZ());
                            double distancia = centroide.calcularDistancia(cercano.getX(), cercano.getY(), cercano.getZ());
                            figura->agregarAristaGrafo(cercano.getIndice(), centroide.getIndice(), distancia);
                            // std::cout << "Grafo despues: " << std::endl;
                            // figura->getGrafo().mostrarGrafo();
                            auto resultado = figura->calcularRutaMasCorta(centroide.getIndice(), indice1);
                            std::cout << "La ruta más corta que conecta el vertice " << indice1 <<
                                " con el centro del objeto " << partes[2] << ", ubicado en (" << centroide.getX()
        ...
        // Comandos con cuatro argumentos
            ...
            if (comando == "ruta_corta") {
                ...
                if (figura != nullptr) {
                    if (partes[1] != partes[2]) {
                        try {
                            ...
                            std::cout << "La ruta más corta que conecta los vértices " << indice1 << " y "
                                     << indice2 << " del objeto " << partes[3] << " pasa por: ";
                            for (int indice : resultado.first) {
                                std::cout << indice << " ";
                            }
                            std::cout << "; con una longitud de " << resultado.second << "." << std::endl;
                ...
                try {
                    ...
                    Vertice v = fig.getVertices()->verticeCercano(stof(partes[1]), stof(partes[2]), stof(partes[3]),
                                                                 0, mejorDistancia);
        ...
        // Comandos con cinco argumentos
            ...
            if (comando == "v_cercano") {
                ...
                float distancia = v.calcularDistancia(x, y, z);
                if (v.getIndice() != -1) {
